# Building OpenHL ADL — L1 draft (JA) — build-along

> openhl SHA `d66b44a`（Stage 10d — auto-deleveraging）に対するドラフト。

## L1 — `openhl-adl-score-ja`
...
````markdown
# レッスン 1 — `AdlScore`, `AdlRecord`, `AdlReport` + `adl_score` — ranking 関数
...
L1 は Module 1 の sortOrder 0 に入る (Module 0 は L0 のみの Orientation):
...
L1 は `d66b44a` (Stage 10d) を引用する。L1 後、`adl.rs` は `score_higher_for_higher_leverage_winner` テストまで答え合わせと一致。`execute_adl` と残りの 16 テスト + 4 proptest は L2 / L3 / L4 で着地。

## 翻訳セルフレビュー（paste 前）

- **L1 は ADL コースで 2 番目に長いレッスン** (35 分)。Justification: 4 つの eligibility predicate それぞれに pedagogical anchor が要る + score formula の 3 つの i128 multiplication + 4 つの型定義。
- **「Newtype is subtractive」フレーミング** が本コースで新規。Liquidation コースの newtype (`MarginRatio`, `Notional` 等) は「value wrapper」として扱われたが、wrap が *なぜ* 存在するかは検証していなかった。ADL の `AdlScore` がより深い framing を導入する正しい場所 — score の *目的* が比較だからだ。
- **冒頭の 4 象限予測 (A/B/C/D)** が読者に math が来る前に score formula の concrete な intuition を与える。L11 of Liquidation と同じ vocabulary-before-mechanism 規律。
- **L1 に「Phase N」構造はない** — 関数が straight (early-return ladder → arithmetic block → return) なので step-by-step で機能する。Phase-N device は L2 の `execute_adl` で復活する。
- **`Option<AdlScore>` vs sentinel が load-bearing concept として扱われる**、単なる implementation detail ではない。L9 の `WithdrawOutcome` (3-variant routing) と L11 の `CloseOutcomeKind` (discriminated dispatch) と connect する — どれも「正しい enum/Option shape が routing 判断を encode する」。
- **`AdlRecord` や `AdlReport` の新規テストなし** — pure data carrier で、L2 / L3 の `execute_adl` テストがそれらを exercise する。L11 で scanner 型が unit test を得なかったのと同じ規律。

### JA 特有のスタイル決定

- **専門用語は英語のまま** (`newtype`, `sentinel`, `tuple struct`, `record struct`, `discriminated dispatch`, `eligibility predicate`, `belt-and-suspenders`, `subtractive` など)。
- **Rust コードと doc コメントは英語のまま**。答え合わせと byte-for-byte 一致のため。
- **`load-bearing` は英語のまま使用。** Liquidation シリーズ全体で確立。
- **「Newtype is subtractive」を意訳しない** — 「Newtype は引き算的」だと意味がぼやける。English のフレーズとしてそのまま残し、文脈で「操作を table から外す」と説明する。
