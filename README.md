...
| **Building with the Stack** | Real-world apps in Rust + Alloy + Revm — the payoff for reading the source | Lesson 1: a complete minimal MEV searcher in ~200 LOC (mempool → fork-sim → arb-detect → bundle). Lesson 2: a reorg-aware Postgres indexer via ExEx in ~250 LOC. More builds coming: custom RPC, wallet backend, EIP-7702 bundler |

**Total: 12 courses (6 × EN + JA), 20 modules, 96 lessons.**
...
# Push schema and seed all 96 lessons
npx prisma db push
npx prisma db seed
...
The seeder lives in `prisma/seed.ts` and pulls from 12 individual course files (`prisma/seed-reth-{beginner,fundamentals,bridge-to-advanced,advanced,expert,building}-{en,ja}.ts`). It clears all course tables and re-creates 12 courses / 20 modules / 96 lessons in one shot.
...
│   └── seed-reth-building-{en,ja}.ts          # 2 lessons each (more coming)
├── src/
