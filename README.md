...
| **Building with the Stack** | Real-world apps in Rust + Alloy + Revm — the payoff for reading the source | Lesson 1: minimal MEV searcher (mempool → fork-sim → arb-detect → bundle). Lesson 2: reorg-aware Postgres indexer via ExEx. Lesson 3: custom RPC endpoint (`txpoolPlus_pendingByGasBucket`) via `extend_rpc_modules`. More coming: wallet backend, EIP-7702 bundler |

**Total: 12 courses (6 × EN + JA), 20 modules, 98 lessons.**
...
# Push schema and seed all 98 lessons
npx prisma db push
npx prisma db seed
...
The seeder lives in `prisma/seed.ts` and pulls from 12 individual course files (`prisma/seed-reth-{beginner,fundamentals,bridge-to-advanced,advanced,expert,building}-{en,ja}.ts`). It clears all course tables and re-creates 12 courses / 20 modules / 98 lessons in one shot.
...
│   └── seed-reth-building-{en,ja}.ts          # 3 lessons each (more coming)
├── src/
