...
export const ACHIEVEMENTS: AchievementDef[] = [
  ...
  // Skills (40-59)
  { id: 40, key: 'python_rookie', name: 'Python Rookie', description: 'Complete your first Python challenge', icon: 'code', category: 'skill' },
  { id: 41, key: 'api_master', name: 'API Master', description: 'Complete all Trading API track courses', icon: 'terminal', category: 'skill' },
  { id: 42, key: 'full_stack_hl', name: 'Full Stack HL', description: 'Complete courses in 3+ tracks', icon: 'layers', category: 'skill' },
...
interface CheckContext {
  ...
  hasPythonChallenge: boolean;
  hasApiTrackComplete: boolean;
  hasSpeedRun: boolean;
...
async function buildContext(userId: string): Promise<CheckContext> {
  ...
  // Check if any completed challenge used Python
  const pythonChallenge = await prisma.lessonProgress.findFirst({
    where: {
      enrollment: { userId },
      isCompleted: true,
      lesson: { type: 'CHALLENGE', challengeLanguage: 'python' },
    },
  });

  // Check Trading API track completion
  const apiCourses = await prisma.course.count({ where: { track: 'trading-api' } });
  const completedApiCourses = completedEnrollments.filter(
    (e) => e.course.track === 'trading-api'
  ).length;
  const hasApiTrackComplete = apiCourses > 0 && completedApiCourses >= apiCourses;
  ...
  // Check early_adopter: joined within first 30 days
  ...
  return {
    ...
    completedTracks,
    hasPythonChallenge: !!pythonChallenge,
    hasApiTrackComplete,
    hasSpeedRun,
    hasPerfectCourse,
    isEarlyAdopter,
...
export async function checkAndUnlockAchievements(userId: string): Promise<number[]> {
  ...
  const checks: [number, boolean][] = [
    ...
    // Skills
    [40, ctx.hasPythonChallenge],            // python_rookie
    [41, ctx.hasApiTrackComplete],           // api_master
    [42, ctx.completedTracks.length >= 3],   // full_stack_hl
