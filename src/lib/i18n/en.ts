export const en = {
  common: {
    appName: 'RethLab',
    tagline: 'Master Reth, Revm, and Alloy',
    signIn: 'Sign In',
    signUp: 'Sign Up',
    signOut: 'Sign Out',
    loading: 'Loading...',
    save: 'Save',
    cancel: 'Cancel',
    back: 'Back',
    next: 'Next',
    previous: 'Previous',
    search: 'Search',
    filter: 'Filter',
    all: 'All',
    viewAll: 'View All',
    learnMore: 'Learn More',
    getStarted: 'Get Started',
    exploreCourses: 'Explore Courses',
    openMenu: 'Open menu',
    closeMenu: 'Close menu',
  },
  nav: {
    home: 'Home',
    courses: 'Courses',
    dashboard: 'Dashboard',
    leaderboard: 'Leaderboard',
    profile: 'Profile',
    settings: 'Settings',
  },
  auth: {
    signInTitle: 'Sign in to RethLab',
    signInSubtitle: 'Continue with your preferred method',
    continueGoogle: 'Continue with Google',
    continueGithub: 'Continue with GitHub',
    devLogin: 'Dev Login',
    email: 'Email',
    signInFailed: 'Sign in failed. Please try again.',
  },
  landing: {
    hero: {
      title: 'Master the Rust Ethereum Stack',
      subtitle: 'A tiered, self-paced path through Reth, Revm, and Alloy — from your first Rust script that hits an RPC, through EVM internals, to building Execution Extensions on a custom Reth node.',
      cta: 'Start Learning',
      ctaSecondary: 'Explore Courses',
    },
    cta: {
      title: 'Ready to Master Reth?',
      description: 'Beginner to advanced — the most direct path into the Rust Ethereum stack used by Paradigm, Hyperliquid, and Tempo.',
    },
    features: {
      title: 'Learn Reth, Revm, and Alloy',
      interactive: {
        title: 'Alloy Fundamentals',
        description: 'Master Ethereum types, signing, providers, and the building blocks every Rust EVM tool relies on.',
      },
      credentials: {
        title: 'Revm Internals',
        description: 'Understand how the EVM actually executes opcodes — stack, memory, gas, and the interpreter loop.',
      },
      gamification: {
        title: 'Hands-On Learning',
        description: 'Interactive quizzes, coding challenges, and real Rust snippets you can run as you go.',
      },
      community: {
        title: 'Reth & ExEx',
        description: 'Build Execution Extensions, custom RPC methods, and learn how to extend Reth like the pros.',
      },
    },
    paths: {
      title: 'Learning Paths',
      subtitle: 'Structured tiers to take you from Rust beginner to Reth contributor.',
      fundamentals: {
        title: 'Beginner',
        description: 'Why the Rust Ethereum stack matters, environment setup, and your first Alloy script.',
      },
      tradingApi: {
        title: 'Alloy & EVM Basics',
        description: 'Addresses, signing, providers, and the EVM concepts you need before reading Revm code.',
      },
      defi: {
        title: 'Revm Internals',
        description: 'The interpreter, opcodes, gas accounting, and the Database trait that powers custom chains.',
      },
      security: {
        title: 'Reth & ExEx',
        description: 'Staged Sync, custom RPCs, and Execution Extensions — the path to production Rust EVM infra.',
      },
      courses: '{{count}} courses',
    },
    stats: {
      courses: 'Courses',
      lessons: 'Lessons',
      tracks: 'Tracks',
      learners: 'Learners',
    },
  },
  courses: {
    categories: {
      basics: 'Basics',
      developer: 'Developer',
      beginner: 'Beginner',
      fundamentals: 'Fundamentals',
      advanced: 'Advanced',
      expert: 'Expert',
    },
    catalog: {
      title: 'Course Catalog',
      subtitle: 'From basics to advanced development — courses to master Hyperliquid.',
      searchPlaceholder: 'Search courses...',
      noResults: 'No courses found matching your filters.',
    },
    difficulty: {
      beginner: 'Beginner',
      intermediate: 'Intermediate',
      advanced: 'Advanced',
    },
    detail: {
      enroll: 'Enroll Now',
      enrolled: 'Continue Learning',
      modules: 'Modules',
      lessons: 'Lessons',
      duration: 'Duration',
      xpReward: 'XP Reward',
      students: 'Students',
      instructor: 'Instructor',
      prerequisites: 'Prerequisites',
      whatYouLearn: "What You'll Learn",
      syllabus: 'Syllabus',
      reviews: 'Reviews',
      reviewsPlaceholder: 'Community reviews coming soon. Complete the course and be the first to share your experience!',
    },
    progress: {
      complete: 'Complete',
      inProgress: 'In Progress',
      notStarted: 'Not Started',
      completedOf: '{{completed}} of {{total}} lessons',
    },
  },
  lesson: {
    title: 'Lesson',
    markComplete: 'Mark Complete',
    completed: 'Completed',
    nextLesson: 'Next Lesson',
    previousLesson: 'Previous Lesson',
    courseComplete: 'Course Complete!',
    courseCompleteMessage: 'You earned {{honey}} XP from this course.',
    backToCourses: 'Browse More Courses',
    hint: 'Show Hint',
    solution: 'Show Solution',
    challenge: {
      title: 'Challenge',
      description: 'Complete this coding challenge:',
      run: 'Run Code',
      submit: 'Submit',
      reset: 'Reset Code',
      testsPassed: '{{passed}} of {{total}} tests passed',
      allPassed: 'All tests passed!',
      outputLabel: 'Output',
      expectedOutput: 'Expected Output',
      expected: 'Expected:',
      actual: 'Actual:',
      passedCount: '{{passed}}/{{total}} passed',
    },
  },
  dashboard: {
    title: 'Dashboard',
    welcome: 'Welcome back, {{name}}!',
    currentCourses: 'Current Courses',
    recentActivity: 'Recent Activity',
    recommended: 'Recommended For You',
    noCourses: 'No courses yet. Start learning!',
    noActivity: 'No recent activity yet.',
    nextLesson: 'Next: {{name}}',
    stats: {
      totalXP: 'Total XP',
      level: 'Level',
      streak: 'Day Streak',
      coursesCompleted: 'Courses Completed',
      rank: 'Global Rank',
    },
    streakMessage: {
      active: "You're on fire! {{days}} day streak",
      broken: 'Start a new streak today!',
      frozen: 'Streak frozen - complete a lesson to continue',
    },
  },
  leaderboard: {
    title: 'Leaderboard',
    subtitle: 'Top learners ranked by XP.',
    timeframes: {
      weekly: 'This Week',
      monthly: 'This Month',
      alltime: 'All Time',
    },
    rank: 'Rank',
    builder: 'Builder',
    xp: 'XP',
    level: 'Level',
    streak: 'Streak',
    you: 'You',
    noRankings: 'No rankings yet. Be the first!',
  },
  profile: {
    title: 'Profile',
    editProfile: 'Edit Profile',
    achievements: 'Achievements',
    completedCourses: 'Completed Courses',
    skills: 'Skills',
    memberSince: 'Member since {{date}}',
    noAchievements: 'No achievements yet. Start learning to earn badges!',
    noCourses: 'No courses completed yet. Keep learning!',
    streakDays: '{{count}}d streak',
    completedOn: 'Completed {{date}}',
    achievementsUnlocked: '{{unlocked}}/{{total}} unlocked',
    lessonsCount: '{{count}} lessons',
    skillArchitecture: 'Architecture',
    skillTradingApi: 'Trading API',
    skillVaults: 'Vaults',
    skillDeltaNeutral: 'Delta-Neutral',
    skillStrategies: 'Strategies',
  },
  settings: {
    title: 'Settings',
    profile: {
      title: 'Profile',
      name: 'Display Name',
      bio: 'Bio',
      avatar: 'Avatar',
      bioPlaceholder: 'Tell us about yourself...',
    },
    account: {
      title: 'Account',
      email: 'Email',
      socialAccounts: 'Social Accounts',
      github: 'GitHub',
      unlink: 'Unlink',
      connect: 'Connect',
      connected: 'Connected',
      notConnected: 'Not connected',
    },
    preferences: {
      title: 'Preferences',
      language: 'Language',
      theme: 'Theme',
      themeLight: 'Light',
      themeDark: 'Dark',
      themeSystem: 'System',
      notifications: 'Notifications',
    },
    privacy: {
      title: 'Privacy',
      profileVisibility: 'Profile Visibility',
      public: 'Public',
      private: 'Private',
      exportData: 'Export Data',
    },
  },
  kodiak: {
    tryKodiak: 'Explore Vaults',
    depositors: 'Vault Users',
    depositDescription: 'Vault users can verify their wallet for a personalized experience.',
    signIn: 'Sign In',
    signInDescription: 'Sign in with Google or GitHub to track your progress across courses.',
    signInToLearn: 'Sign in to learn',
    verify: 'Verify',
    verified: 'Verified! Unlocking courses...',
    notFound: 'No deposit found.',
    gateTitle: 'Start Learning on RethLab',
    gateSubtitle: 'Sign in to continue learning.',
    ctaTitle: 'Put this into practice',
    ctaSubtitle: 'DN + Funding Rate strategies on Hyperliquid',
    completionTitle: 'Course Complete!',
    completionSubtitle: 'Ready to put your skills to work? Explore automated vault strategies.',
    completionFeatures: 'DN + Funding Rate,Yield Strategies,Active Risk Mgmt',
    lessonCta: 'Explore Vaults',
    widgetCta: 'Explore Vaults',
  },
  honey: {
    title: 'Earn XP, Unlock Courses',
    subtitle: 'XP unlocks advanced courses as you progress.',
    tier1: {
      title: 'Beginner Courses',
      description: 'Beginner courses are open from the start.',
    },
    tier2: {
      title: 'Intermediate Courses',
      description: 'Unlock Vault Development and advanced protocol courses.',
    },
    tier3: {
      title: 'Advanced Courses',
      description: 'Unlock Keepers & Bots and Advanced Strategies courses.',
    },
    tier4: {
      title: 'Coming Soon',
      description: 'More perks and integrations coming soon.',
    },
    needMore: 'More XP Needed',
    requirement: 'This course requires {{needed}} XP. You currently have {{current}} XP.',
    earnMore: 'Complete more lessons to earn XP',
  },
  quiz: {
    progress: '{{current}} / {{total}}',
    correct: 'correct',
    checkAnswer: 'Check Answer',
    next: 'Next Question',
    seeResults: 'See Results',
    passed: 'Quiz Passed!',
    failed: 'Not quite yet',
    passedMessage: 'Great job! You can move on to the next lesson.',
    failedMessage: 'You need 70% to pass. Review the material and try again.',
    retry: 'Try Again',
  },
  footer: {
    stayUpdated: 'Stay Updated',
    newsletterDescription: 'Get notified about new courses and platform updates.',
    subscribe: 'Subscribe',
    subscribed: 'Subscribed!',
    emailPlaceholder: 'your@email.com',
    builtBy: 'Built by Fabrknt.',
    disclaimer: 'RethLab is a Fabrknt project. All content is for educational purposes. Code samples are simplified for learning — verify against the official Reth, Revm, and Alloy repositories before using in production.',
    learn: 'Learn',
    resources: 'Resources',
    community: 'Community',
  },
  gamification: {
    xp: 'XP',
    level: 'Level {{level}}',
    nextLevel: '{{xp}} XP to Level {{level}}',
    lvl: 'Lvl {{level}}',
    streak: {
      title: 'Streak',
      days: '{{count}} days',
      milestone7: 'Week Warrior',
      milestone30: 'Monthly Master',
      milestone100: 'Consistency King',
    },
    achievements: {
      title: 'Achievements',
      firstSteps: 'First Steps',
      courseCompleter: 'Course Completer',
      speedRunner: 'Speed Runner',
      weekWarrior: 'Week Warrior',
      monthlyMaster: 'Monthly Master',
      pythonRookie: 'Python Rookie',
      apiMaster: 'API Master',
      fullStackHL: 'Full Stack Perp',
      earlyAdopter: 'Early Adopter',
      bugHunter: 'Bug Hunter',
      perfectScore: 'Perfect Score',
    },
  },
  about: {
    title: 'About RethLab',
    subtitle: 'RethLab is a Fabrknt project — a tiered learning path through the Rust Ethereum stack used by Paradigm, Hyperliquid, and Tempo.',
    bio: 'The Rust Ethereum stack — Reth, Revm, and Alloy — sits at the heart of the most performant chains being built today. But the documentation is scattered, the source code is dense, and there is no clear on-ramp from "I know some Rust" to "I can write an ExEx." RethLab is that on-ramp: tiered, opinionated, and written from the perspective of someone who learned by doing.',
    background: {
      tradfi: 'Why Rust',
      tradfiDetail: 'Memory safety, zero-cost abstractions, and an ecosystem mature enough to power serious blockchain infrastructure.',
      blockchain: 'Why Reth',
      blockchainDetail: 'A modular, blazing-fast Ethereum client from Paradigm — the foundation many App-chains build on.',
      vaults: 'Why Revm',
      vaultsDetail: 'The Rust EVM that powers Reth, Foundry, and most modern simulation tooling.',
      multilingual: 'Why Alloy',
      multilingualDetail: 'The next-generation type-safe library for Ethereum in Rust — the modern replacement for ethers-rs.',
    },
    built: {
      title: 'What This Covers',
      kodiak: 'Beginner & Fundamentals: Why the Rust EVM stack matters, environment setup, your first Alloy script, EVM as a stack machine, Revm intro, and Foundry (forge / cast / anvil).',
      rethlab: 'Advanced & Expert: Reading the actual Revm interpreter, custom opcodes & precompiles, the Database trait, Staged Sync, ExEx, NodeBuilder for App-chains, MDBX, MPT, MEV pipelines, zkEVM with Steel, and shipping a custom Reth fork.',
    },
    tech: {
      title: 'Why This Stack?',
      subtitle: 'Reth, Revm, and Alloy are quietly becoming the standard for serious Ethereum infrastructure.',
      signal: 'Reth: Modular Rust Ethereum client — faster sync, lower memory, and the basis of many App-chains.',
      regime: 'Revm: World-class Rust EVM — used inside Reth, Foundry, OP-Reth, and most zkVMs.',
      crossvenue: 'Alloy: Type-safe Ethereum primitives — the modern replacement for ethers-rs.',
      liquidation: 'ExEx: Reth Execution Extensions — hook directly into the execution loop in Rust.',
      tilt: 'Compose: Use Alloy to talk to any chain, drive Revm in process, or extend Reth as a node operator.',
      hyperlend: 'Career value: Infra engineers who can read this stack are some of the highest-paid in Web3.',
    },
    contact: 'Questions or feedback? Reach out on Twitter.',
  },
  errors: {
    notFound: 'Page not found',
    notFoundMessage: "This page doesn't exist.",
    backHome: 'Back to Home',
    unauthorized: 'Please sign in to continue',
    serverError: 'Something went wrong',
    networkError: 'Network error. Please try again.',
    loadCourses: 'Failed to load courses. Please try again.',
    loadDashboard: 'Failed to load dashboard. Please sign in and try again.',
    loadLeaderboard: 'Failed to load leaderboard. Please try again.',
    loadProfile: 'Failed to load profile. Please sign in and try again.',
    runCode: 'An error occurred while running your code.',
    loadLesson: 'Failed to load lesson. Please try again.',
    loadCourse: 'Failed to load course. Please try again.',
  },
  page: {
    launchBanner: 'Read any lesson without signing up — sign in to track your progress',
    whyRethLab: {
      title: 'Why RethLab?',
      subtitle: 'A direct, tiered path through the Rust Ethereum stack.',
      builtByBuilder: 'Tiered Path',
      builtByBuilderDesc: 'Beginner → Fundamentals → Advanced. No jumping into ExEx code on day one — each tier builds on the last.',
      learnByDoing: 'Learn by Doing',
      learnByDoingDesc: 'Real Rust snippets you can copy into Rust Playground or `cargo run`, plus quizzes that test understanding, not memorization.',
      growingPlatform: 'Source-First',
      growingPlatformDesc: 'Pointers into the actual Reth, Revm, and Alloy source so you graduate from reading docs to reading code.',
      allCoursesFree: 'Open Access',
      allCoursesFreeDesc: 'Every lesson is readable without an account. Sign in only when you want to track progress and earn XP.',
    },
    tracks: {
      title: '3 Tiers, One Stack',
      subtitle: 'From your first Rust script to writing Execution Extensions on a custom Reth node.',
      basicsDesc: 'Why the Rust EVM stack matters, environment setup, and your first Alloy script that hits a real RPC.',
      fundamentals: 'Fundamentals',
      fundamentalsDesc: 'Alloy types, providers, signing, and the EVM concepts (stack, memory, opcodes, gas) you need before reading Revm.',
      hyperliquid: 'Advanced',
      hyperliquidDesc: 'Revm interpreter internals, custom opcodes, the Database trait, Reth Staged Sync, and Execution Extensions.',
      expertDesc: 'Performance engineering, MDBX storage, Tokio internals, procedural macros, custom precompiles, MPT, MEV pipelines, zkEVM, and shipping a custom Reth fork.',
      hlDev: 'Reth Devs',
      hlDevDesc: 'Reth Staged Sync, custom RPC methods, ExEx, and the Reth SDK for building App-chains.',
      beginner: 'Beginner',
      beginnerToIntermediate: 'Beginner → Intermediate',
      beginnerToAdvanced: 'Beginner → Advanced',
      courseCount: '{{count}} courses',
    },
    about: {
      insightTitle: 'The Insight Behind RethLab',
      insightDesc: "Reth, Revm, and Alloy are some of the most exciting projects in Ethereum infrastructure — but the on-ramp is brutal. The official docs assume you already know the architecture. The source code is dense and uses heavy generics. RethLab exists to be the missing middle layer: a structured, opinionated path from `cargo new` to writing an ExEx.",
      whatsNext: "What's Next",
      moreContent: 'More Content',
      moreContentDesc: 'Deeper Revm internals (precompiles, EOF), Reth SDK walkthroughs, and ExEx case studies.',
      multiLanguage: 'Multi-Language',
      multiLanguageDesc: 'Available in English and Japanese, with parity across both. More languages planned as the community grows.',
      moreCourses: 'More Courses',
      moreCoursesDesc: 'As the Rust Ethereum ecosystem evolves — new opcodes, new ExEx patterns, new chains — new lessons will follow.',
      rethlabDesc: 'A tiered learning path through Reth, Revm, and Alloy. Beginner to advanced, in English and Japanese. Built from the perspective of someone learning the stack alongside you.',
      flowDesc: 'Hyperliquid flow intelligence — real-time capital flow signals, regime detection, narrative rotation, and weekend gap monitoring. Not a price predictor — a capital intention visualizer.',
    },
    signals: {
      title: 'Signals',
      subtitle: 'Funding rates and regime detection on Hyperliquid.',
      refresh: 'Refresh',
      lastUpdated: 'Last updated:',
      error: 'Failed to load signals. Please try again.',
      regime: 'Regime',
      bestFunding: 'Best Funding',
      crossVenue: 'Funding Rates (Annualized)',
      openInterest: 'Open Interest',
      howToRead: 'How to Read These Signals',
      fundingComparison: 'Funding Rate Comparison',
      fundingComparisonDesc: 'Compare funding across Hyperliquid, Binance, and Bybit. Large divergence between DEX and CEX rates signals arbitrage opportunity or impending convergence.',
      hypeOnly: 'HYPE (HL-Only)',
      hypeOnlyDesc: 'HYPE is only traded on Hyperliquid — no CEX comparison available. Funding is structurally positive due to holding incentives but can reverse during market stress.',
      regimeLabel: 'Regime',
      regimeDesc: 'Calm = normal. Elevated = watch. Volatile = active stress. Extreme = crisis. Based on the same thresholds used by Kodiak vaults.',
      bestFundingLabel: 'Best Funding',
      bestFundingDesc: 'Shows the highest funding rate assets on Hyperliquid. Useful for identifying DN strategy opportunities.',
      severityNone: 'None',
      severityLow: 'Low',
      severityHigh: 'High',
      severityCritical: 'Critical',
    },
    consulting: {
      deployTitle: 'Need Help Deploying?',
      deployDesc: 'RethLab teaches the patterns. The specific parameters — thresholds, timing, risk matrices — come from operational experience that can\'t be generalized. If you\'re building a vault or automated strategy and need help with the details, DM us.',
      strategyTitle: 'Need Strategy Consulting?',
      strategyDesc: 'Understanding these strategies is one thing. Deploying capital with the right parameters, risk controls, and operational setup is another. If you\'re evaluating DN strategies for your portfolio, DM us.',
      cta: 'DM @fabrknt on X',
      signalsDesc: 'Want custom alert thresholds, private API access, or strategy consulting? DM us on X.',
    },
  },
  time: {
    justNow: 'Just now',
    minutesAgo: '{{count}}m ago',
    hoursAgo: '{{count}}h ago',
    daysAgo: '{{count}}d ago',
  },
};

// Use recursive string type instead of literal types so translations can differ
type DeepStringRecord<T> = {
  [K in keyof T]: T[K] extends string ? string : DeepStringRecord<T[K]>;
};

export type TranslationKeys = DeepStringRecord<typeof en>;
