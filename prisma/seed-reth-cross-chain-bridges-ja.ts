...
**5 件中 3 件が鍵/署名の侵害**。スマートコントラクトロジックではなく、運用セキュリティの失敗だ。
...
クロスチェーンの最終チェック。Tempo、Hyperliquid、あるいは任意の Reth ベース L1 に触れる bridge を設計するために必要な内容だ。`,
                  quizQuestions: [
                    {
                      question: 'なぜ **過去の top 5 bridge ハックのうち 3 件** がスマートコントラクトのバグではなく鍵 / 運用侵害だったのか?',
                      options: [
                        'スマートコントラクトは形式検証されているのでバグは稀だから。',
                        'Multisig bridge はセキュリティを鍵保持者に依存する; 鍵の十分なサブセット (phishing、malware、内部者) が侵害されれば、コントラクトのコードは無関係になる — 署名は正しく検証されてしまう。運用セキュリティが最弱リンクなのだ。',
                        'ハッカーは取引所のような容易なターゲットを好むから。',
                        'スマートコントラクトハックは bridge ハックに分類されないから。',
                      ],
                      correctIndex: 1,
                      explanation: 'Ronin ($625M)、Orbit ($80M)、Poly Network ($611M 返却) — いずれも鍵侵害。Bridge のコードは本来やるべきこと (署名検証) を正しく行った。攻撃が成功したのは鍵そのものが盗まれたからだ。これが、multisig が「機能の選択」ではなく根本的なリスクモデルである理由。',
                    },
                    {
