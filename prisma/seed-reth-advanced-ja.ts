...
Fundamentals は終えている前提です（理想的には Revm Advanced と Bridge to Advanced も）。Beginner と Fundamentals のレッスンは、あなたに寄り添って歩いてきました。**Reth Advanced は違います。** この短いオリエンテーションで違いを把握し、レッスン 1 から正しいマインドセットで臨めるようにしましょう。

## このコースは何で、何ではないか

このコースは **Reth のソースを 1 行ずつ読み解く** ためのもの。具体的には: \`Stage\` トレイトと 10 ステージの sync パイプライン、**ExEx (Execution Extensions)** — インプロセスのインデクサ・MEV・リスクエンジン用、そして自前の App-chain を組み立てる **Reth SDK**。

3 つの並行する Advanced コースの 1 つで、順番は自由です:
...
Reth Advanced レッスンは、これまでのティアにはない **能動学習プロンプト** を使います：
...
**Rust が不安?** このコース内に *Rust: ライフタイム・Box・Arc・dyn Trait* レッスンがあるが、それは *テスト* であって *教える* ものではありません。Bridge to Advanced コースが教える側。
...
各主要トピック（Staged Sync・ExEx・Reth SDK）はそれぞれ **3〜4 本のアトミックなレッスンの「チェーン」** に分割されています：
...
各アトミックレッスンは表示上 **8〜12 分、プロンプトに本気で取り組むと〜15〜25 分**。1 つのトピックチェーン全体（積み上げ → ウォークスルー → クイズ → ドリル、3〜4 本）は **おおよそ 45〜80 分** — 1 晩がかりの仕事。Expert はもっと長い。

**正しい単位はセッションあたり 1 チェーン**。チェーン（例えば Staged Sync の 4 本）を始めたら、その日のうちに終わらせる計画で。積み上げ・ウォークスルー・クイズが互いに強化し合い、ドリルで「読み」が「記憶」に変わる。チェーンを跨日で割る — 今晩は積み上げの半分、来週に残りとドリル — はモデルを壊します。

時間がないなら: 2 本を流すよりも *1 本を丁寧に*。このティアではエンゲージメントが網羅性より重要。

## 準備完了

前提リストに見覚えがあって、リポジトリも開いている: コース詳細に戻って **「\`Stage\` トレイトをステップで組み立てる」** から始める。

前提リストが不安だった: Revm Advanced か Bridge to Advanced に戻る。

Reth Advanced を終えたら、**Alloy Advanced**（未受講なら）か、手続きマクロや zkVM 統合の深掘りがある **Expert** へ。`,
                },
              ],
            },
          },
