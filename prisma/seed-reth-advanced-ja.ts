...
このレッスンでは、最も素朴な同期ループからこのトレイトを積み上げていきます。最後まで通せば、以下の全要素を自分の手で組み立てたことになる:
...
このレッスンでは、最も素朴な「ブロックリスナー」から API を積み上げていきます。最後まで通せば、本物の最小 ExEx の全要素を自分で組み立てたことになる — 次のレッスンでそれを詳細に読みます。
...
\`ChainReverted\` で \`-=\` を忘れると、カウントは永久に膨らみ続けます。\`ChainReorged\` で \`-=\` を忘れると、カウントは canonical ではなく、古いチェーンと新しいチェーンの和を表すことになる。
...
一致していれば、インデクサは reorg-safe です。**本番グレードのインデクサ（goldsky、the graph など）が出荷しているのと同種のコードを書いたことになる。**
...
このドリルを終えれば、reorg-safe でノード速度のインデクサを出荷したことになる。**同じ道具で MEV ボット、リアルタイムリスクエンジン、ロールアップも作れます。**`,
                },
                {
...
コンポーネントが、依存する型がまだ確定していない時点で指定されることになる。各コンポーネントビルダーを未確定の型でジェネリックにする（コンパイラと戦う羽目になる）か、\`.with_components\` の引数で型を暗黙に commit する（静かに混乱を招く）かしかない。**\`with_types\` を先頭に置く** ことで、残りのチェーンを型認識可能にしています。
...
ゲートを通過したら: **Alloy → Revm → Reth（Staged Sync、ExEx、カスタム NodeBuilder）** の階段を上ってきたことになる。3 プロジェクトすべてのソースを目的を持って読めます。
