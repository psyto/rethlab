...
export async function seedRethConsensusEngineeringEN(prisma: PrismaClient) {
  const tags = ['consensus', 'bft', 'pos', 'reth', 'advanced'];
  ...
  await prisma.course.create({
    data: {
      ...
      description:
        'Build and read production L1 consensus. Twelve lessons across three modules: BFT fundamentals + three consensus families + Ethereum PoS + HotStuff/HyperBFT (Module 1), reading Reth\'s Consensus trait + Malachite + bera-reth (Module 2), wiring custom consensus through NodeBuilder + building a 100-line minimal single-leader BFT + validator economics (Module 3). By the end you can defend any L1 consensus design and read any production consensus codebase.',
      difficulty: 'ADVANCED',
      duration: 193,
      xpReward: 530,
        ...
        create: [
          {
            ...
            lessons: {
              create: [
                {
                  title: 'Lesson 1 — The BFT problem from scratch',
                  slug: 'consensus-bft-problem-en',
                  type: 'CONTENT',
                  sortOrder: 0,
                  duration: 16,
                  xpReward: 40,
                  content: `# Lesson 1 — The BFT problem from scratch
...
`,
                ...
                {
                  title: 'Lesson 2 — Three consensus families — PoW, PoS, classical BFT',
                  slug: 'consensus-three-families-en',
                  type: 'CONTENT',
                  sortOrder: 1,
                  duration: 16,
                  xpReward: 40,
                  content: `# Lesson 2 — Three consensus families — PoW, PoS, classical BFT
...
`,
                ...
                {
                  title: 'Lesson 3 — Ethereum\'s PoS — Casper FFG + LMD-GHOST',
                  slug: 'consensus-ethereum-pos-en',
                  type: 'CONTENT',
                  sortOrder: 2,
                  duration: 18,
                  xpReward: 45,
                  content: `# Lesson 3 — Ethereum's PoS — Casper FFG + LMD-GHOST
...
`,
                ...
                {
                  title: 'Lesson 4 — HotStuff and HyperBFT — the single-leader BFT family',
                  slug: 'consensus-hotstuff-hyperbft-en',
                  type: 'CONTENT',
                  sortOrder: 3,
                  duration: 17,
                  xpReward: 45,
                  content: `# Lesson 4 — HotStuff and HyperBFT — the single-leader BFT family
...
`,
          ...
          },
          {
            ...
            lessons: {
              create: [
                {
                  title: 'Lesson 5 — Reading Reth\'s Consensus trait',
                  slug: 'consensus-reth-trait-en',
                  type: 'CONTENT',
                  sortOrder: 0,
                  duration: 18,
                  xpReward: 45,
                  content: `# Lesson 5 — Reading Reth's Consensus trait
...
`,
                ...
                {
                  title: 'Lesson 6 — Reading Malachite — Rust-native BFT by Informal Systems',
                  slug: 'consensus-malachite-en',
                  type: 'CONTENT',
                  sortOrder: 1,
                  duration: 18,
                  xpReward: 45,
                  content: `# Lesson 6 — Reading Malachite — Rust-native BFT by Informal Systems
...
`,
                ...
                {
                  title: 'Lesson 7 — Reading bera-reth — Proof-of-Liquidity as consensus customization',
                  slug: 'consensus-bera-reth-en',
                  type: 'CONTENT',
                  sortOrder: 2,
                  duration: 15,
                  xpReward: 40,
                  content: `# Lesson 7 — Reading bera-reth — Proof-of-Liquidity as consensus customization
...
`,
          ...
          },
          {
            ...
            lessons: {
              create: [
                {
                  title: 'Lesson 8 — NodeBuilder consensus slot — wiring custom consensus',
                  slug: 'consensus-nodebuilder-slot-en',
                  type: 'CONTENT',
                  sortOrder: 0,
                  duration: 16,
                  xpReward: 45,
                  content: `# Lesson 8 — NodeBuilder consensus slot — wiring custom consensus
...
- **NodeBuilder pattern.** \`NodeBuilder::new().pool(...).consensus(...).network(...).executor(...)\` — every major slot is overridable.
- **Consensus slot integration.** \`NodeBuilder::consensus(your_impl)\` — registers your \`Consensus\` trait impl; Reth uses it everywhere.
- **\`Consensus\` impl skeleton.** ~80 lines for a minimal impl: validate_header (parent hash + timestamp + gas limit), validate_block (txs + state root + receipts root), validate_state (post-execution state root).
- **Validator-set source.** Your impl reads from somewhere — staking contract / chainspec / on-chain registry. Cache for performance.
...
- **Integration test.** Spin up a Reth node with your \`Consensus\` impl + execute a few blocks + assert no validation errors.
- **Production examples.** OP-Reth (uses \`OptimismConsensus\`), bera-reth (uses \`BeraConsensus\` with PoL), Tempo (custom). Same NodeBuilder pattern; different Consensus impl.
...
- NodeBuilder consensus slot = \`NodeBuilder::consensus(your_impl)\`. Custom Consensus impl is registered; Reth uses it everywhere.
- ~80 lines for a minimal Consensus impl (validate_header / validate_block / validate_state). Validator set from staking contract / chainspec / registry.
...
`,
                ...
                {
                  title: 'Lesson 9 — Building a minimal single-leader BFT in Rust',
                  slug: 'consensus-minimal-bft-en',
                  type: 'CONTENT',
                  sortOrder: 1,
                  duration: 20,
                  xpReward: 50,
                  content: `# Lesson 9 — Building a minimal single-leader BFT in Rust
...
- **Five components.** State machine (Driver) + vote tallying (VoteKeeper) + leader selection + network broadcast + a \`CentralizedSequencer\` for testing.
- **Driver = state machine.** \`enum Phase { Propose, Prevote, Precommit, Commit }\`; transitions on quorum or timeout. ~50 lines.
- **VoteKeeper.** \`HashMap<(height, round, phase), Vec<(Address, Signature)>>\` + check 2/3 stake-weighted threshold. ~30 lines.
- **Leader selection.** Round-robin: \`leader = validators[round % n]\`. Simple; production uses VRF or stake-weighted.
- **Network broadcast.** Tokio mpsc channels for local testing; replace with \`libp2p::gossipsub\` or RLPx for production.
- **\`CentralizedSequencer\` for tests.** A test harness that owns all validators and drives them through phases. Asserts that all reach commit after 4 rounds.
- **Add to NodeBuilder.** Wrap the Driver in a \`Consensus\` trait impl + register via \`NodeBuilder::consensus(...)\` — now Reth runs your BFT.
- **~100 lines = minimum viable BFT.** Production-grade adds: bandwidth control + Byzantine attack defence + slashing + reorg handling. Each is its own lesson.
...
`,
                ...
                {
                  title: 'Lesson 10 — Validator economics — slashing, rewards, attack vectors',
                  slug: 'consensus-validator-economics-en',
                  type: 'CONTENT',
                  sortOrder: 2,
                  duration: 17,
                  xpReward: 45,
                  content: `# Lesson 10 — Validator economics — slashing, rewards, attack vectors
...
`,
                ...
                {
                  title: 'Quiz — Building L1 Consensus',
                  slug: 'consensus-final-quiz-en',
                  type: 'QUIZ',
                  sortOrder: 3,
                  duration: 12,
                  xpReward: 50,
                  content: `# Quiz — Building L1 Consensus
