...
それから、コースを 1 つ選んで始める。`,
                ...
                {
                  title: 'Advanced コースの読み方',
                  slug: 'advanced-tier-orientation-ja',
                  type: 'CONTENT',
                  sortOrder: 4,
                  duration: 6,
                  xpReward: 15,
                  content: `# Advanced コースの読み方

あなたは Advanced ティアの入口に立っています — **3 つの独立したコース**（**Revm Advanced**・**Reth Advanced**・**Alloy Advanced**）はすべて同じスキルを教えます: **本番の Rust ソースを 1 行ずつ読む。** この短いレッスンは、それらのコースがどう構成されているかを説明し、レッスン 1 から正しいマインドセットで臨めるようにします — どの Advanced コースから始めても。

(各 Advanced コースにも、コース固有の前提とセットアップを扱う短いウェルカムがあります。*このレッスン* は、1 度読めば済む **汎用的な** オリエンテーション。)
...
編集スタイルはこれまでのチュートリアルより難しい。意図的にそうしています。**滑らかなチュートリアル散文は「わかった気になる」trap を作る** — 読者は複雑な内容を流して読み、浅い理解で去ってしまう。Predict / Recall / Quiz パターンは実際のエンゲージメントを強制します。

これは **設計上の摩擦**。受け入れて。

## レッスンの実際の読み方

各主要 Advanced トピック（\`add\`・カスタム Opcode・Database トレイト・Staged Sync・ExEx・Reth SDK・Provider トレイト）はそれぞれ **3〜4 本のアトミックなレッスンの「チェーン」** に分割されています：
...
1. 解説の前に 🛑 **Predict** プロンプトに当たる。止まる。頭の中か紙で答える。それから続ける。
2. 解説を読む。予測と比較。**間違っていた箇所が学びの場所。**
3. 🔍 **Find-in-repo** プロンプトに当たる。リポジトリを開く。実際のファイルを見つける。レッスンの主張を検証。
...
各アトミックレッスンは表示上 **8〜12 分、プロンプトに本気で取り組むと〜15〜25 分**。1 つのトピックチェーン全体（積み上げ → ウォークスルー → クイズ → ドリル、3〜4 本）は **おおよそ 45〜80 分** — 1 晩がかりの仕事。**L1 Architect ティア（Advanced の先）** のチェーンはさらに長い — 1 セッションに 1.5〜3 時間が普通。実際の L1 アーキテクトの仕事が要求する長さだから。
...
3 つの Advanced コースは独立していて任意の順序で進められます — ただし **Revm Advanced を最初に推奨**、その型（\`Address\`、\`U256\`、\`Database\` トレイト）が Reth と Alloy の前提になるから:
...
## Advanced の先 — L1 Architect ティア

Advanced を完走すると、次は **L1 Architect ティア**（5 コース、難易度 **EXPERT**、track \`reth-l1-architect\`）が待っています。Advanced が「本番ソースを **読む**」を教えるなら、L1 Architect は「自分で L1 を **設計する**」を教える — Hyperliquid や Tempo クラスの L1 をアーキテクトするための実装スキル。
...
各チェーンは Advanced の 1.5〜3 倍の長さ。**今は気にしないこと** — まず Advanced のチェーンを 1 つ完走するのが先。L1 Architect は遠い目的地としてだけ意識しておけば十分。
