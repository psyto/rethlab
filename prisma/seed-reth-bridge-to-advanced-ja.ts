...
dispatch loop で opcode が *何* かは見えました。ほとんどの opcode は 4 つのストアのうちの 1 つに触れます。本レッスンではそれらを順に見ていきます。あわせて、Solidity が隠しているものの中級レッスンが前提とするワールドステートのモデルも確認します。
...
捉え方の反転は、**Rust ではどの値も、ある時点で所有者がただ 1 つだけ** という点です。所有者だけが借用を配ることができ、\`&mut\` の借用は 1 つだけ、または \`&\` の借用が任意の数で、両方を同時に成立させることはできません。これが GC なしで並行コードを安全に保つ仕組みで、最初の 2〜3 週間で文法を重く感じさせる正体でもあります。
...
- **\`Result<T, E>\` はただの enum** です。魔法はありません。関数は \`Ok(value)\` か \`Err(reason)\` のどちらかを返し、呼び出し側は両方の腕を扱うか、\`?\` 演算子で伝播する必要がある。コンパイラはエラーを無視することを許しません — Solidity でいえば「revert の処理を忘れたらコンパイルエラー」に近い厳しさです。
