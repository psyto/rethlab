...
Fundamentals は終えている前提です（理想的には Bridge to Advanced も）。Beginner と Fundamentals のレッスンは、あなたに寄り添って歩いてきました。**Revm Advanced は違います。** この短いオリエンテーションで違いを把握し、レッスン 1 から正しいマインドセットで臨めるようにしましょう。

## このコースは何で、何ではないか

このコースは **Revm のソースを 1 行ずつ読み解く** ためのもの。それだけ — EVM 実行エンジンを Rust で読みます。

3 つの並行する Advanced コースの最初の 1 つで、順番は自由です:
...
Revm Advanced は別のことを要求します。**Revm の本物の本番ソースを 1 行ずつ読みます。** レッスンはあなたが既にこのスタックを使えることを前提にしている — *読めるように* 教えます。
...
- **クイズゲート** — すべての主要トピックは 4〜5 問のクイズで終わり、進行をゲートします。**クイズはうなずきで通せない** — それが設計。2 問以上落としたら積み上げが内面化していない、戻る。
...
(Reth Advanced 内に *Rust: ライフタイム・Box・Arc・dyn Trait* レッスンがあるが、それは *テスト* であって *教える* ものではありません。Bridge コースが教える側。)
...
Advanced の主要トピック（\`add\`、カスタム Opcode、Database トレイト、Staged Sync、ExEx、Reth SDK）はそれぞれ **3〜4 本のアトミックなレッスンの「チェーン」** に分割されています：
...
**正しい単位はセッションあたり 1 チェーン**。チェーン（例えば \`add\` の 4 本）を始めたら、その日のうちに終わらせる計画で。積み上げ・ウォークスルー・クイズが互いに強化し合い、ドリルで「読み」が「記憶」に変わる。チェーンを跨日で割る — 今晩は積み上げの半分、来週に残りとドリル — はモデルを壊します。
