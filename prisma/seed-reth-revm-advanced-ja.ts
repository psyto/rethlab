...
> 🔍 **\`revm/src/host.rs\` を開いて、\`dyn Host\` が実際に作られている箇所を1つ見つけてください。** この \`?Sized\` opt-out が複雑さに見合うだけの価値を持つ具体的な根拠になります。
...
> **🧭 ここまでで進んだ場所:** **VM 層の opcode をひとつ**、本物の Revm ソースと機能的に等価な水準まで、\`IT: ITy\`、\`?Sized\`、その場スタック操作、\`wrapping_add\` の 4 行で組み上げた。次のレッスンで本体をマクロに抽出する — CPython・JVM が何十年もやってきたボイラープレート削減の EVM 版。
`,
                },
                {
...
同じ形: チェック → 失敗側に cold ヒント → 早期リターン。ガスを引いて、払えなければ \`OutOfGas\` で即時終了。\`popn_top!\` を消化していれば、\`gas!\` は同じパターンの5行版です。
...
これは一般化します。コストがオペランドに形作られる Opcode は本体内で課金しなければなりません。覚えておいてください — \`sha3\`、\`mload\`、\`call\` 系で同じパターンに出会います。
...
> **🧭 ここまでで進んだ場所:** **VM 層の命令ディスパッチ機構** を完成 — 256 スロット const テーブル + \`Instruction<W,H>\` ラッパで、コンパイル時に O(1) ディスパッチを保証。CPython のバイトコードディスパッチャや JVM のインタプリタテーブルと同じ形。次のレッスンで自分の opcode をこのテーブルに挿入する — ディスパッチ面そのものが拡張点。
`,
                },
                {
...
> 🔍 **走らせるとどうなるかを推論。** 自分のカスタム Opcode 入りの Reth ノードを起動し、素の geth を同じチェーンヘッドに向けたら: geth はいつ切断する? 答え: \`0x0C\` を含むブロックを実行しようとした時点。geth は \`0x0C\` を INVALID として実行、あなたの Reth は swap として実行 → ブロックの state-root 検証が失敗。「ブロックを共有できない」は読み流すのではなく、*この具体例で腑に落とす* べき主張です。
...
修正は古典的な **依存性逆転**（Revm 自身が「具体的なストレージ実装」を抱える代わりに、抽象トレイトを通して問い合わせる側に回る — 「具体に依存」を「抽象に依存」に逆転させる設計）です。Revm にストレージを所有させず、インターフェース経由で *問い合わせさせる*。トレイトを定義して、Revm が状態に必要とすることを *記述する* — ストレージを所有せずに:
...
直接的には解決しない。\`Arc<T>\` は \`&T\` しか出さない、\`&mut T\` は出さない。\`Database\` のメソッドは \`&mut self\` を取るので、\`Arc<MyDb>\` は \`Database\` を実装できない。**この事実が、トレイトを 2 つに分ける設計判断を必然にする** — 次レッスンで解決します: Revm は \`Arc\` のために *仲間の* 読み専用トレイト（\`DatabaseRef\`）を持っています。
...
> **🧭 ここまでで進んだ場所:** **VM 層と DB 層の境目**（読み API）を完成 — 4 メソッド + 関連型 \`Error\` + \`auto_impl\`。同じ Revm が、VM をいっさい触らずに、インメモリ Map・リモート JSON-RPC・MDBX・シャード網のどれにも乗る形になった。次のレッスンで読み / 書き分離を開ける — \`Arc\` を使えるようにし、eager / lazy fetch を分けるための設計判断。
`,
                },
                {
...
どれか間違えたら、\`Database\` × EVM セマンティクスのメンタルモデルを作り直す必要があります — 組み立てレッスンを読み直してからドリルを続けてください。
