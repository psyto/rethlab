...
2. **The \`?\` operator inside the closure is the elegant part.** \`adl_score(s, mark)?\` short-circuits the closure when \`adl_score\` returns \`None\`, propagating it out of \`filter_map\`. Without \`?\`, you'd need an explicit \`match\` or \`let Some(score) = ... else { return None }\`. **The \`?\` operator works inside any \`-> Option<_>\` closure, not just async functions.**
...
Determinism. \`sort_unstable_by\` is faster (no extra allocation, no insertion-order tracking) but the *order of equal elements is unspecified*. In a single validator's process, that's fine. Across validators on different machines, even with the same input, different runtime ordering could produce different \`AdlReport\`s if scores happen to be equal. Stable sort guarantees that the relative order of equal-scored records is the same on every validator. **The \`_unstable_\` variant trades determinism for speed; in consensus code, determinism wins.**
