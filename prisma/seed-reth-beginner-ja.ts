...
「Solanaも高速で、しかもRustだから、わざわざRust EVMを学ぶ意味は？」とよく聞かれます。結論は **「目指す方向性次第」** ですが、判断軸を整理しておく。
...
「**現在のトレンドと汎用性**」を重視するなら、**Reth系（このコース）** が答えです。Solanaは依然として強いですが、Rust EVMは「EVM の開発体験 × Rustの性能」という両取りができる稀有なポジションにあります。

## 2026年の現実 — 「どちらか」ではなく「交差点」

最近の構図は、単純な二択ではありません。要点は次の3つです。
...
Rust EVM側（このコース）の主戦場は、app-chain（Hyperliquid）、stablecoin決済（Tempo）、L2（Base / OP-Reth）です。Rethは「Stripe of crypto」型インフラの基盤になりつつあります。
...
プロジェクトの地図は前のレッスンで描きました。次に出てくる質問はこれです — **なぜチームは古い選択肢から積極的に移行しているのか?** Geth は 10 年間 Ethereum を走らせてきました。ethers-rs は長らく Rust Ethereum ライブラリの定番でした。それでも、新しいインフラは Reth と Alloy の上に作られています。その理由を、置き換え 1 件ずつ見ていく。
...
決済優先の独自トランザクション順序付けを持つ L1 をチームが ship する場面を想像してみてください。fork するのは Reth で、それも全体ではありません。Reth の crate に依存したまま、\`Pool\` と \`Payload\` コンポーネントだけを差し替えます。Geth なら全コードベースを fork し、永遠に続くリベース税を払い、自分では触りたくない 20 万行のコードを抱え込むことになる。これがまさに Tempo がやっていることで、前のレッスンに並んだ Reth ベース L1 すべてが採用している形です。
...
\`cargo\` は **Rustのパッケージマネージャー＋ビルドツール** です。npmやcargoという名前から「ライブラリを管理するもの」と思いがちですが、実際は **ビルドもテストも実行もすべてcargoでやる** ので、毎日触ることになる。
