...
カスタムプロトコルは、新たな攻撃面を晒すことになります。デフォルトの eth/68 には実戦で鍛えられた防御策が組み込まれていますが、自作プロトコルには、自分で書くまで何もありません。最低限、押さえておきたい項目は次の通り:
...
HL のネットワークに参加するノードを構築したいなら、彼らの独自プロトコルを理解する必要があります。公開仕様ではありませんが、パターンとしては同じ筋 — RLPx 系の transport の上に乗せたカスタムサブプロトコルです。
