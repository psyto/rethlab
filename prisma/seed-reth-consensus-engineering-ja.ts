...
n=100 で 1 ブロックあたり 10,000 メッセージ。n=1000 で 100 万。スケールしない。
...
**賛成論**: Day 1 から経済的セキュリティが効き、「我々を信頼してください」期間がなくなる
...
最終的なコンセンサスのチェック。Tempo クラスの L1 を出荷するために必要。`,
                  quizQuestions: [
                    ...
                    },
                    {
                      ...
                      correctIndex: 1,
                      explanation: 'ネットワークがボトルネックになる。PBFT の n² メッセージは、n=100 で 1 万、n=1000 で 100 万メッセージ。HotStuff の O(n) で楽にはなるが、帯域とレイテンシのせいで実用上の最大値は依然上限がある。Ethereum 系のハイブリッド PoS は、fork choice (毎 slot、サンプリングベース) と finality (毎 epoch、フル参加) を分けることでこれを解決している。',
                    },
                    {
