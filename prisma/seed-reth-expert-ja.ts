...
性能チューニング、デッドロックのデバッグ、ステージが突然止まった理由の診断 — どれも、ノード自身が「何をしているか」を **教えてくれる** ことが前提になります。Reth がこの問いに用意した答えが \`tracing\`、Rust エコシステムの構造化ログ用 crate です。Reth の主要なコードパスは例外なく \`tracing\` の span と event で計装されていて、可観測性まわり（ログ・メトリクス・分散トレース）はすべてこの一枚岩の基盤の上に乗っています。本レッスンは、その仕組みと拡張方法を扱います。

> 📌 **なぜ Performance & Systems モジュールに置いてあるか。** 計測できないものは最適化できないからです。\`tracing\` が計測層です。続く性能レッスン（flamegraph、MDBX チューニング、Tokio ランタイム）は、稼働中のノードから正しい信号を引き出せる前提で進みます。その引き出し口がこの \`tracing\` です。

## 1. \`tracing\` crate の 2 つのプリミティブ

エコシステム全体は、2 つの発想に集約されます。

- **Span** — 実行の *領域* を表す印。「ブロック 100..200 について \`SenderRecoveryStage::execute\` の中にいる」というような範囲を表します。span は入れ子になり、中のすべてのログ行に文脈（ブロック範囲、peer ID、リクエスト ID）を持ち回らせます。
- **Event** — *ある時点* のログ行。「バッチ 17 を 12,000 件の sender でコミットした」のような一行です。event は現在アクティブな span の中で発火し、その文脈を引き継ぎます。
...
> 🛑 **スクロール前に予測。** カスタム *Opcode* はメインネットとのコンセンサスを破る (中級で見た通り)。カスタム *Precompile* は破らない — 同じく vanilla EVM になかった新コードなのに。**なぜ答えが違う?** EVM バイトコードパーサーを引いて仮説を立ててください。
...
フォークを ship したとします。カスタム precompile、独自の payload builder、いじったガススケジュール。ユニットテストは通っている。前のレッスンの「バニラ Reth に対する diff テスト」は、*変更した* 部分が *変更していない* パスでも以前と同じに振る舞うことを教えてくれます。**ですが、*変更していない* パスが、あなたの変更によって壊れていないことはどう知ればいいのでしょうか?** さらに難しいのは、*誰もテストを書かなかったパス* に潜むバグを見つけることです。

答えは、**2 つの方向からの自動的な正しさの圧力** です。\`execution-spec-tests\` で仕様への準拠を構造的に保証し、**differential fuzzing** で誰も探そうと思わなかったバグを浮かび上がらせる。本番で Revm/Reth フォークを動かしている L1 チームは、全員この 2 つを回しています。本レッスンはその方法を扱います。

> 📌 **位置付け。** Inside REVM の *Revm 自身のテスト* のレッスンは、フォーマット（state tests、EOF tests、execution-spec-tests）の説明でした。**本レッスンはその本番運用版** — チェーンチームがこれらの道具を *vanilla Revm に対してではなく、自分たちのフォークに* どう適用するか、を扱います。

## 1. execution-spec-tests を自分のフォークに適用する

vanilla Revm は upstream の EEST スイートを通ります。あなたのフォーク — 異なるガススケジュール、独自 precompile、独自 ChainSpec を持つもの — も **同じことを示す必要があります**。EEST をフォークのバイナリに対して走らせ、不一致を「意図的な仕様逸脱（その旨を文書化する）」か「バグ（修正する）」のどちらかに振り分けていきます。
...
これがプリコンパイル層にあって純粋 EVM ではない理由は性能。AES-256-GCM を EVM バイトコードで実装すると、ネイティブ実装よりおよそ 100× 遅い。Tempo チームは速度向上が劇的に大きい場所だけプリコンパイル化した。象徴的な例: **HKDF-SHA256 (ECDH 共有秘密から AES 鍵を導出するのに使う) はプリコンパイルでは *ない*。** スペックから:
...
Reth のアーキテクチャ全体は二つ目のモデルを軸に組まれています。中級コースで見た NodeBuilder / components / ChainSpec パターンは、まさに **reth のソースを patch せずに chain を出荷する** ために存在しているのです。
